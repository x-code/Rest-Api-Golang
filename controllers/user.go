...
func (uc UserController) CreateUser(c *gin.Context) {
	...
	u := uc.create_user(json.Name, json.Photo,json.Age,c)
    if u.Name == json.Name {
        content := gin.H{
            "result": "Success",
            "Name": u.Name,
...
// UpdateUser an existing user resource
func (uc UserController) UpdateUser(c *gin.Context) {
	...
	u := uc.update_user(id,json.Name, json.Photo,json.Age,c)
...
// UploadImage creates a photo resource
func (uc UserController) UploadImage(c *gin.Context) {
    ...
    urlPhoto := baseUrl + "/assets/" + file.Filename
    oid := bson.ObjectIdHex(id)
    // Stub user
    get := models.User{}
    uc.session.DB(DB_NAME).C(DB_COLLECTION).FindId(oid).One(&get)
    uc.upload_image(id,get.Name,urlPhoto,get.Age,c)
...
func (uc UserController) create_user(Name string, Photo string,Age int, c *gin.Context) models.User {
    user := models.User{
        Name:      Name,
        Photo:    Photo,
        Age:	Age,
...
func (uc UserController) update_user(Id string,Name string, Photo string,Age int, c *gin.Context) models.User {
...
func (uc UserController) upload_image(Id string,Name string,Photo string,Age int, c *gin.Context) models.User {
